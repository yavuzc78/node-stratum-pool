...
const BitcoinCashAddress = require('bchaddrjs');
...
/**
 * Converts bitcoin cash address to legacy format
 * f.e. bitcoincash:qzkfallx0nwxr6ary4700l60dzygzkkm9utndve4gt -> 1GjjhUvddQ8rxhZRL1ofh4RHHpH5TaiYAB
 */
function convertBcashAddressToLegacy(addr) {
    return BitcoinCashAddress.toLegacyAddress(addr);
}


exports.getVersionByte = function(addr){
    if (!addr) {
        throw new Error("Address is null: " + addr);
    }

    try {
        if(BitcoinCashAddress.detectAddressFormat(addr) === BitcoinCashAddress.Format.Cashaddr) {
            addr = convertBcashAddressToLegacy(addr);
        }
    } catch(e) {
    }
